...
def test_natural_search_requires_no_auth(monkeypatch):
    ...
    monkeypatch.setattr("backend.routers.search_router.resolve_search_query", lambda q: ("cmc <= %s", [3.0]))
    monkeypatch.setattr("backend.routers.search_router.cards.search_cards", lambda conn, sql, params: [{"id": "c1"}])
...
def test_natural_search_passes_query_text_through(monkeypatch):
    ...
    monkeypatch.setattr("backend.routers.search_router.resolve_search_query", mock_resolve)
    monkeypatch.setattr("backend.routers.search_router.cards.search_cards", lambda conn, sql, params: [])
